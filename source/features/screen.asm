...
; Color Constants for Enhanced Visuals
.COLOR_DARK_BLUE     equ 00000001b
.COLOR_GREEN         equ 00000010b
.COLOR_CYAN          equ 00000011b
.COLOR_RED           equ 00000100b
.COLOR_MAGENTA       equ 00000101b
.COLOR_BROWN         equ 00000110b
.COLOR_LIGHT_GRAY    equ 00000111b
.COLOR_DARK_GRAY     equ 00001000b
.COLOR_BRIGHT_BLUE   equ 00001001b
.COLOR_BRIGHT_GREEN  equ 00001010b
.COLOR_BRIGHT_CYAN   equ 00001011b
.COLOR_BRIGHT_RED    equ 00001100b
...
os_draw_background:
	...
	mov ah, 09h			; Draw gradient bar at top
	mov bh, 0
	mov cx, 80
	mov bl, 00100111b		; Dark blue on grey
	mov al, ' '
	int 10h
	...
	mov ah, 09h			; Draw gradient bar at bottom
	mov bh, 0
	mov cx, 80
	mov bl, 00100111b		; Dark blue on grey
	mov al, ' '
...
os_dialog_box:
    ...
    ; Add shadow effect
    mov dh, 9           ; Shadow starts one row above
    mov dl, 20
    call os_move_cursor
    ...
    mov bh, 0
    mov cx, 42
    mov bl, 00001000b   ; Dark gray for shadow
    mov al, ' '
    int 10h
    popa

    mov dh, 9			; First, draw teal background box
	mov dl, 19
...
.redbox:				; Loop to draw all lines of box
	...
	mov bh, 0
	mov cx, 42
	mov bl, 00110011b		; Teal/Cyan color scheme with slight shadow effect
	mov al, ' '
	int 10h
	popa
...
.two_button:
	mov bl, 01110000b		; Dark grey on white for buttons
	mov dh, 14
	mov dl, 35
	mov si, 8
	mov di, 15
...
os_draw_gradient_background:
    pusha

    mov dx, 0           ; Start from top-left
    call os_move_cursor

    mov ah, 6           ; Scroll function
    mov al, 0           ; Clear entire screen
    mov bh, 00011011b   ; Default gradient (dark blue to cyan)
