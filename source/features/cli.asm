; ==================================================================
; ForgeOS -- The Forge Operating System kernel
; Copyright (C) 2006 - 2019 ForgeOS Developers -- see doc/LICENSE.TXT
;
...
get_cmd:				; Main processing loop
	...
	mov di, create_string		; 'CREATE' entered?
	call os_string_compare
	jc near create_file

	mov di, append_string		; 'APPEND' entered?
	call os_string_compare
	jc near append_file
...
.filename_provided:
	; Prepare a sample buffer with some bytes
	mov si, .sample_content
	mov di, 32768			; Use standard buffer location
	mov cx, .sample_content_end - .sample_content
	rep movsb			; Copy sample content to buffer

	; Write the file with sample content
	mov ax, command			; Filename from command
	mov bx, 32768			; Buffer with sample content
	mov cx, .sample_content_end - .sample_content	; File size
	call os_write_file
	jc .write_fail
...
.write_fail:
	...
	.sample_content	db 'Hello, ForgeOS!', 13, 10, 'This is a test file.', 13, 10
	.sample_content_end:

	.success_msg	db 'File created successfully. Size: ', 0
...
.filename_provided:
	mov cx, 64			; Maximum input length
	mov di, file_append_buffer	; Buffer to store input
	mov byte [di], 0		; Ensure buffer starts empty

	; Prompt for content to append
	mov si, .append_prompt
	call os_print_string

	mov ax, file_append_buffer
	mov bx, cx
	call os_input_string

	; Check if input is empty
	mov si, file_append_buffer
	cmp byte [si], 0
	je get_cmd			; Nothing to append, return to command prompt

	; Load existing file
	mov ax, command			; Use the original filename
	mov bx, 32768			; Load into memory
	mov cx, 32768			; Maximum file size
	call os_load_file

	; If file doesn't exist, start with empty buffer
	jc .new_file

	; bx now contains the size of loaded file
	push bx				; Save original file size

	; Append new content
	mov si, file_append_buffer
	add di, bx			; Move to end of existing content
	call os_string_copy

	pop bx				; Restore original file size
	call os_string_length		; Get length of appended content
	add bx, ax			; Total new file size

.write_file:
	mov ax, command			; Filename
	mov cx, bx			; Total file size
	mov bx, 32768			; Buffer with combined content
	call os_write_file
	jc .write_fail
...
.write_fail:
	...
	.append_prompt	db 'Enter text to append (press Enter to finish):', 13, 10, 0
	.success_msg	db 'File appended successfully', 13, 10, 0
...
; =====================================================================
	...
	file_append_buffer	times 256 db 0
	...
	help_text		db 'Commands: DIR, CREATE, APPEND, LS, COPY, REN, DEL, CAT, SIZE, CLS, HELP, TIME, DATE, VER, EXIT', 13, 10, 0
	...
	version_msg		db 'ForgeOS ', FORGEOS_VER, 13, 10, 0
